...
lockstep ::
  ...
  At Resp m r ->
  Event m r
lockstep model@Model{..} cmdErr (At resp) =
  Event
    { eventBefore = model
    , eventCmdErr = cmdErr
    , eventAfter = model'
...
shrinker m (At (CmdErr mbErrors cmd)) =
  fmap At $
    ...
    CmdErr (Just errors) cmd -> do
      ...
      res <-
        withErrors (unsafeToUncheckedStrictTVar varErrors) errors $
          tryImmutableDB (Proxy @TestBlock) $
            run env cmd
      case res of
...
-- error is simulated.
simulatedError ::
  (Event m Symbolic -> Either Tag (EventPred m)) ->
  EventPred m
...
tag =
 ...
 where
  ...
  tagGetBlockComponentEmptySlot = successful $ \ev r -> case r of
    ErAllComponents (Left (EmptySlot{}))
      | GetBlockComponent{} <- unAt $ eventCmdNoErr ev ->
  ...
  tagGetBlockComponentWrongHash = successful $ \ev r -> case r of
    ErAllComponents (Left (WrongHash{}))
      | GetBlockComponent{} <- unAt $ eventCmdNoErr ev ->
  ...
  tagGetBlockComponentNewerThanTip = successful $ \ev r -> case r of
    ErAllComponents (Left (NewerThanTip{}))
      | GetBlockComponent{} <- unAt $ eventCmdNoErr ev ->
  ...
  tagIteratorStreamedN ::
    Map IteratorId Int ->
    EventPred m
  tagIteratorStreamedN streamedPerIterator =
    C.Predicate
...
execCmds ::
  forall m.
  Model m Symbolic ->
  QSM.Commands (At CmdErr m) (At Resp m) ->
  [Event m Symbolic]
