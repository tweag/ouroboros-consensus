  ...
  case someHeaderInFutureCheck cfgEnv of
    InFutureCheck.SomeHeaderInFutureCheck headerInFutureCheck ->
      ChainSyncClientPipelined $
        continueWithState () $ -- Start ChainSync by looking for an intersection between our
    ...
    traceException $ do
      ...
      (theirFrag, theirHeaderStateHistory) <- do
        case attemptRollback
          ...
          (ourFrag `withTime` ourHeaderStateHistory, ourHeaderStateHistory) of
    ...
    Stateful $ \kis ->
      traceException $
        let KnownIntersectionState
         ...
         in case attemptRollback
              rollBackPoint
              (theirFrag, theirHeaderStateHistory) of
              Nothing ->
  ...
  checkArrivalTime kis arrival = do
    Intersects kis' (lst, judgment) <- do
      readLedgerState kis $ \lst ->
        case runExcept $
          judgeHeaderArrival (configLedger cfg) lst arrival of
...
checkValid cfgEnv intEnv hdr hdrSlotTime theirTip kis ledgerView = do
  ...
  theirHeaderStateHistory' <-
    case runExcept $
      validateHeader cfg ledgerView hdr hdrSlotTime theirHeaderStateHistory of
