...
staticCandidates GenesisTest{gtSecurityParam, gtGenesisWindow, gtBlockTree} =
 ...
 where
  ...
  selection branch =
    AF.takeOldest
      (AF.length (btbPrefix branch) + fromIntegral (unNonZero $ maxRollbacks gtSecurityParam))
      (btbFull branch)
...
firstBranch peers =
  fromMaybe newest $
    minimumBy (compare `on` forkAnchor)
      <$> nonEmpty (filter hasForked (toList (adversarialPeers'' peers)))
 where
  newest =
    maximumBy
      (compare `on` (AF.headSlot . candidate . value))
      (toList (honestPeers'' peers) ++ toList (adversarialPeers'' peers))
...
prop_densityDisconnectMonotonic =
 ...
 where
  check final events = \case
   ...
   where
    withEvents
      | debug = counterexample (catLines debugInfo)
      | otherwise = id

    debugInfo =
      "Event log:"
        : ((++ [""]) . prettyUpdateEvent =<< events)
        ++ ["k: " ++ show k']
  ...
  forAllGenesisTest
    ...
    ( \GenesisTest{gtBlockTree, gtSchedule} stateView@StateView{svTipBlock} ->
        let
         ...
         in
          counterexample "Unexpected exceptions" exnCorrect
