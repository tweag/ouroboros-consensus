...
import           Ouroboros.Consensus.Block (HasHeader, pointSlot, SlotNo, WithOrigin (..), realPointSlot, ConvertRawHash, StandardHash, CodecConfig, NestedCtxt, Header)
import           Ouroboros.Consensus.Storage.Common (StreamFrom (..), StreamTo (..), BlockComponent)
import           Ouroboros.Consensus.Storage.ImmutableDB.API (ImmutableDB (..), getTipPoint)
import           Ouroboros.Consensus.Storage.ImmutableDB.Chunks.Internal (ChunkNo (..), ChunkInfo)
...
import           Ouroboros.Consensus.Util.IOLike (IOLike, StrictTVar, readTVar, readTVarIO, writeTVar, atomically, newTVarIO, modifyTVar, NoThunks)
...
decorateImmutableDB ::
     ...
     , ConvertRawHash blk
     , StandardHash blk
     , Typeable blk, NoThunks OnDemandState
     )
        ...
        { stream_ = \registry component from to -> do
            ...
            if tipPoint >= toPoint
                then stream_ db registry component from to
                    ...
                    Right <$> Lightweight.mkLightweightIterator 
                        odcHasFS 
                        odcChunkInfo 
                        odcCodecConfig 
                        odcCheckIntegrity 
