...
1.  **Excessive Load:** Stake pool relays must serve hundreds of gigabytes of historical data to new syncing nodes.
...
The **Genesis Sync Accelerator (GSA)** is a tool designed to alleviate network burden on stake pool relays and to make syncing faster for a new node or a node which has not been active for some time. The GSA does this bridging low-cost, centralized Content Delivery Networks (CDNs) and the decentralized Ouroboros network.

In particular, GSA functions as a proxy server (extending `immdb-server`) which:
...
This approach uses commodity object storage (e.g., S3, Cloudflare R2) to lighten the load on nodes in the P2P network, while maintaining the trustless properties of Ouroboros Genesis.
...
[ Syncing Node ]  <-- (Ouroboros N2N) -->  [ Genesis Sync Accelerator ]  <-- (HTTPS) -->  [ CDN / Object Storage ]
      |                                           (immdb-server)                                  |
      |                                                 |                                         |
      +--- (Verifies Chain)                             +--- (Lightweight Reader)                 +--- (Stores files)
```
...
4.  **CDN Fetch:** If the requested blocks are unknown, the relevant chunk files are downloaded from the CDN.
...
*   `.secondary`: Secondary index containing block offsets and hashes.
*   `.primary`: Primary index mapping slots to secondary offsets.

### 3.2 Server-Side Implementation (`immdb-server`)

#### 3.2.1 Component: `RemoteStorage`
Responsible for the HTTP transport layer. It ensures that for a requested chunk number, the trio of files (`.chunk`, `.primary`, `.secondary`) is present in the local cache directory.
...
3.  If the range ends before the main DB's indexed tip -> **Delegate** to main DB.
4.  If the range goes beyond the tip:
...
*   **Direct Access:** It opens the `.secondary` and `.chunk` files directly using the `fs-api`.
